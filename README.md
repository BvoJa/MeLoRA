...
Đây là mã nguồn demo cho bài toán fine-tuning hiệu quả tham số bằng **LoRA** và **MELoRA** trên các tác vụ NLP tiếng Việt và benchmark NLU. Thư mục này tập trung vào PhoBERT cho **Vietnamese Sentiment Analysis** trên dataset `uitnlp/vietnamese_students_feedback`, đồng thời giữ lại script VietMed-NER, GLUE và LLaMA từ repo gốc.
...
| File | Mục đích |
| --- | --- |
| `CS221/peft-0.5.0/` | Bản PEFT tùy chỉnh có hỗ trợ MELoRA. Cần cài đặt editable trước khi chạy. |
| `CS221/run_vietnamese_students_feedback_phobert.py` | Script huấn luyện/evaluate/predict PhoBERT cho Sentiment Analysis trên Vietnamese Students Feedback. |
| `CS221/vietnamese_students_feedback_phobert.sh` | Script bash cấu hình sẵn thí nghiệm PhoBERT + MELoRA cho dataset `uitnlp/vietnamese_students_feedback`. |
| `CS221/run_vietmed_ner_phobert.py` | Script huấn luyện/evaluate/predict PhoBERT cho VietMed-NER. |
| `CS221/vietmed_ner_phobert.sh` | Script bash cấu hình sẵn thí nghiệm PhoBERT + MELoRA cho VietMed-NER. |
| `CS221/run_glue_lora.py`, `CS221/glue_finetune.sh` | Script LoRA/MELoRA cho GLUE benchmark. |
| `CS221/llama_finetune.py`, `CS221/llama_finetune.sh` | Script instruction tuning với LLaMA. |
| `TrainMLM_CS221_*.ipynb`, `TrainNER_Med.ipynb` | Notebook huấn luyện MLM/NER phục vụ thí nghiệm môn học. |
| `prediction_comparison.json`, `test_predictions_*.json` | File kết quả dự đoán và so sánh prediction đã sinh trong quá trình chạy thử. |
| `CS221/figs/method.png` | Hình minh họa phương pháp MELoRA. |
| `CS221/requirements.txt` | Thư viện Python cần cài đặt. |
...
| `vietmed_ner_phobert.sh` | Vietnamese medical NER | `vinai/phobert-base` | `leduckhai/VietMed-NER` | 80 epochs, batch 64, max length 256, F1 |
...
- `test_predictions.txt`: dự đoán trên test set cho script VietMed-NER.
- log W&B offline, nếu `--report_to wandb` được bật.

Metric chính của demo Sentiment Analysis là **Macro F1** và **Accuracy**. Metric chính của demo NER là **F1**. GLUE dùng metric riêng theo task, ví dụ `pearson` cho STS-B và `matthews_correlation` cho CoLA.
